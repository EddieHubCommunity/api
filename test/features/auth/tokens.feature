...
Feature: auth module
    ...
    Scenario: get list of all tokens in invalid keyspace
        Given authorisation
        When make a GET request to "/auth/tokens/invalid-keyspace"
    ...
    Scenario: fail to create a token with no authorisation
        When make a POST request to "/auth/tokens" with:
            | serverId | "eddiehub"    |
            | scopes   | ["Data.Read"] |
    ...
    Scenario: delete token
        ...
        When make a POST request to "/auth/tokens" with:
            ...
            | scopes   | ["Data.Read"] |
        Then the response status code should be 201
        When make a DELETE request to "/auth/tokens/{clientId}" with:
            | serverId | "eddiehub"    |
            | scopes   | ["Data.Read"] |
